...
- **Deliberately not in Sprint 1** (see the `NOTE:` comments in the
  relevant routers): auth endpoints and anything gated by the Human
  Approval Layer (`/topics/{id}/approve`, `/articles/{id}/approve`, ...) —
  Sprint 2; anything that triggers an LLM call (`/campaigns/{id}/start`,
  `/topics/{id}/generate-brief`, `/competitor-pages/{id}/analyze`, ...) and
  the job worker — Sprint 3; SEO audit execution + the `reports` router —
  Sprint 4; Playwright publish automation — Sprint 5.
